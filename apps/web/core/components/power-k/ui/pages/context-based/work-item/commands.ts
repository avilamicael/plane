    ...
    async (formData: Partial<TIssue>) => {
      ...
      await updateEntity(workspaceSlug.toString(), entityDetails.project_id, entityDetails.id, formData).catch(() => {
        setToast({
          ...
          message: `${isEpic ? t("common.epic") : t("common.work_item")} could not be updated. Please try again.`,
        });
      });
    },
      ...
      onSelect: (data) => {
        ...
        try {
          ...
          setToast({
            type: TOAST_TYPE.ERROR,
            title: t("common.toast.error"),
            message: `${entityDetails.is_epic ? t("common.epic") : t("common.work_item")} could not be updated. Please try again.`,
          });
        }
      },
