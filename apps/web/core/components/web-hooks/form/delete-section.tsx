      ...
      {({ open }) => (
        <div className="w-full">
          <Disclosure.Button as="button" type="button" className="flex w-full items-center justify-between py-4">
            <span className="text-16 tracking-tight">{t("danger_zone")}</span>
            ...
            <Disclosure.Panel>
              <div className="flex flex-col gap-8">
                <span className="text-13 tracking-tight">
                  {t("workspace_settings.settings.webhooks.delete_warning")}
                </span>
                <div>
                  <Button variant="error-fill" size="lg" onClick={openDeleteModal}>
                    {t("workspace_settings.settings.webhooks.delete_webhook")}
                  </Button>
                </div>
              </div>
            </Disclosure.Panel>
