...
export const IssueBlockCycle = observer(function IssueBlockCycle({ cycleId, shouldShowBorder = true }: Props) {
  ...
  const cycleName = cycle?.name ?? t("common.no_cycle");

  return (
    <Tooltip tooltipHeading={t("common.cycle")} tooltipContent={cycleName}>
      <div
        className={cn(
      ...
      >
        <div className="flex w-full items-center gap-1.5 text-11">
          ...
          <div className="max-w-40 truncate">{cycleName}</div>
        </div>
      </div>
    </Tooltip>
  );
