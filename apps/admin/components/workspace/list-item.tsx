      ...
      <div className="flex items-start gap-4">
        ...
        >
          {workspace?.logo_url && workspace.logo_url !== "" ? (
            <img
              ...
              alt={t("admin.settings.workspace.logo_alt")}
            />
          ) : (
            (workspace?.name?.[0] ?? "...")
          )}
        ...
        <div className="flex flex-col items-start gap-1">
          <div className="flex w-full flex-wrap items-center gap-2.5">
            ...
            <Tooltip tooltipContent={t("admin.settings.workspace.unique_url_tooltip")}>
              <h4 className="text-13 text-tertiary">[{workspace.slug}]</h4>
            </Tooltip>
          </div>
          {workspace.owner.email && (
            <div className="flex items-center gap-1 text-11">
              <h3 className="font-medium text-secondary">{t("admin.settings.workspace.owned_by")}</h3>
          ...
          <div className="flex items-center gap-2.5 text-11">
            {workspace.total_projects !== null && (
              <span className="flex items-center gap-1">
                <h3 className="font-medium text-secondary">{t("admin.settings.workspace.total_projects")}</h3>
            ...
            {workspace.total_members !== null && (
              <>
                •
                <span className="flex items-center gap-1">
                  <h3 className="font-medium text-secondary">{t("admin.settings.workspace.total_members")}</h3>
